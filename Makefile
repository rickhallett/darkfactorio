.PHONY: test gate-sample gate-sample-adversarial build-dfgate build-dfgatev01 build-dflearn build-dfwindowv01 build-dfcorpusv01 build-dffactoryv04 build-dffactoryv05 build-dfstressv04 build-dfshadowv01 build-dfonboardv01 build-dfpromotionv01 learning-touch learning-check window-advance window-advance-high corpus-adversarial factory-v04-validate factory-v05-validate stress-v04 shadow-pack onboard-project onboard-validate promotion-gate-local
...
build-dfpromotionv01:
	$(GO) build -o ./bin/dfpromotionv01 ./cmd/dfpromotionv01
...
promotion-gate-local:
	$(GO) run ./cmd/dfpromotionv01 --run-stress $(or $(RUN_STRESS),false) --output text
